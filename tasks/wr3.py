...
class Wr3(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        feet_names = [name for name in body_names if 'calf' in name]
        self.feet_indices = torch.zeros(len(feet_names), dtype=torch.long, device=self.device, requires_grad=False)
        knee_names = [name for name in body_names if "thigh" in name]
        ...
        for i in range(self.num_envs):
            ...
            anymal_handle = self.gym.create_actor(env_ptr, anymal_asset, start_pose, "wr3", i, 1, 0)
